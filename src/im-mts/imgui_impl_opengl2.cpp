...
static const ImDrawIdx* ImGui_ImplOpenGL2_SetDrawData(const ImDrawList* cmd_list) {
...
static float ImGui_ImplOpenGL2_ScratchMemory[1024];
...
void ImGui_ImplOpenGL2_RenderDrawData(ImDrawData* draw_data)
{
	...
	for (int n = 0; n < draw_data->CmdListsCount; n++)
	{
		...
		const ImDrawIdx* idx_buffer = ImGui_ImplOpenGL2_SetDrawData(cmd_list);
		...
		for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
		{
			...
			if (pcmd->UserCallback)
			{
				...
				if (pcmd->UserCallback == ImDrawCallback_ResetRenderState) {
					ImGui_ImplOpenGL2_SetupRenderState(draw_data, fb_width, fb_height);
					ImGui_ImplOpenGL2_SetDrawData(cmd_list);
				}
				else if (pcmd->UserCallback == ImDrawCallback_NoBlending) {
					glDisable(GL_BLEND);
				}
