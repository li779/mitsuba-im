...
# "git", setting the following variables:
...
#                      code date, in the format nnnnn.nn.nn[gitXXXXXXXXXXXX]
...
function(MTS_GET_VERSION_INFO)
  ...
  find_program(GIT_CMD git DOC "Git command line executable")
  mark_as_advanced(GIT_CMD)
  if (GIT_CMD)
    execute_process(
      COMMAND "${GIT_CMD}" log -1 --format=format:%H,%ad --date=short
      OUTPUT_VARIABLE GIT_INFO
      OUTPUT_STRIP_TRAILING_WHITESPACE
    )
    if (GIT_INFO)
      # Extract the revision ID and the date
      string(REGEX REPLACE "(.+),.+" "\\1" MTS_REV_ID "${GIT_INFO}")
      string(REGEX REPLACE ".+,(.+)-(.+)-(.+)" "\\1.\\2.\\3" MTS_DATE "${GIT_INFO}")
  ...
  if (MTS_DATE MATCHES "([0-9]+)\\.([0-9]+)\\.([0-9]+)")
    ...
    if(MTS_HAS_VALID_REV)
      set(MTS_MACLS_VERSION "${MTS_MACLS_VERSION}git${MTS_REV_ID}")
    endif()
